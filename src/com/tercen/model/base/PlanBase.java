...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.Plan_CLASS ? m.get(Vocabulary.KIND) : null);
...
public static Plan createFromJson(LinkedHashMap m) {return PlanBase.fromJson(m);} 
public static Plan fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
case Vocabulary.Plan_CLASS :
return new Plan(m);
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class Plan in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.Plan_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.Plan_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
