...
public class UserSecretBase extends PersistentObject {
	...
	public UserSecretBase(LinkedHashMap m) {
		...
		this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String) m.get(Vocabulary.SUBKIND)
				: (String) (m.get(Vocabulary.KIND) != Vocabulary.UserSecret_CLASS ? m.get(Vocabulary.KIND) : null);
	...
	public static UserSecret createFromJson(LinkedHashMap m) {
		return UserSecretBase.fromJson(m);
	}

	public static UserSecret fromJson(LinkedHashMap m) {
		String kind = (String) m.get(Vocabulary.KIND);
		switch (kind) {
		case Vocabulary.UserSecret_CLASS:
			return new UserSecret(m);
		default:
			throw new IllegalArgumentException("bad kind : " + kind + " for class UserSecret in fromJson constructor");
		}
	}

	public LinkedHashMap toJson() {
		LinkedHashMap m = super.toJson();
		m.put(Vocabulary.KIND, Vocabulary.UserSecret_CLASS);
		if (this.subKind != null && this.subKind != Vocabulary.UserSecret_CLASS)
