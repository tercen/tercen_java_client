...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.ProjectDocument_CLASS ? m.get(Vocabulary.KIND) : null);
this.projectId = (String)m.get(Vocabulary.projectId_DP) ;
this.folderId = (String)m.get(Vocabulary.folderId_DP) ;
}

public static ProjectDocument createFromJson(LinkedHashMap m) {return ProjectDocumentBase.fromJson(m);} 
public static ProjectDocument fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
...
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class ProjectDocument in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.ProjectDocument_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.ProjectDocument_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
