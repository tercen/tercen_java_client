...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.ModelStep_CLASS ? m.get(Vocabulary.KIND) : null);
}

public static ModelStep createFromJson(LinkedHashMap m) {return ModelStepBase.fromJson(m);} 
public static ModelStep fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
...
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class ModelStep in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.ModelStep_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.ModelStep_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
