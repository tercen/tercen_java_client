...
public class DataStepBase extends CrossTabStep {
	...
	public static DataStep createFromJson(LinkedHashMap m) {
		return DataStepBase.fromJson(m);
	}

	public static DataStep fromJson(LinkedHashMap m) {
		String kind = (String) m.get(Vocabulary.KIND);
		switch (kind) {
		case Vocabulary.DataStep_CLASS:
			return new DataStep(m);
		default:
			throw new IllegalArgumentException("bad kind : " + kind + " for class DataStep in fromJson constructor");
		}
	}

	public LinkedHashMap toJson() {
		LinkedHashMap m = super.toJson();
		m.put(Vocabulary.KIND, Vocabulary.DataStep_CLASS);
		if (this.subKind != null && this.subKind != Vocabulary.DataStep_CLASS)
