...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.Activity_CLASS ? m.get(Vocabulary.KIND) : null);
...
if (m.get(Vocabulary.date_OP) == null) this.date = new Date();
else this.date = DateBase.fromJson((LinkedHashMap)m.get(Vocabulary.date_OP) );
...
public static Activity createFromJson(LinkedHashMap m) {return ActivityBase.fromJson(m);} 
public static Activity fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
case Vocabulary.Activity_CLASS :
return new Activity(m);
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class Activity in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.Activity_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.Activity_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
