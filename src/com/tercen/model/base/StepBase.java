...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.Step_CLASS ? m.get(Vocabulary.KIND) : null);
...
if (m.get(Vocabulary.state_OP) == null) this.state = new StepState();
else this.state = StepStateBase.fromJson((LinkedHashMap)m.get(Vocabulary.state_OP) );
}

public static Step createFromJson(LinkedHashMap m) {return StepBase.fromJson(m);} 
public static Step fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
...
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class Step in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.Step_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.Step_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
