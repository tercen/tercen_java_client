...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.CSVTask_CLASS ? m.get(Vocabulary.KIND) : null);
...
if (m.get(Vocabulary.schema_OP) == null) this.schema = new Schema();
else this.schema = SchemaBase.fromJson((LinkedHashMap)m.get(Vocabulary.schema_OP) );
if (m.get(Vocabulary.params_OP) == null) this.params = new CSVParserParam();
else this.params = CSVParserParamBase.fromJson((LinkedHashMap)m.get(Vocabulary.params_OP) );
}

public static CSVTask createFromJson(LinkedHashMap m) {return CSVTaskBase.fromJson(m);} 
public static CSVTask fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
case Vocabulary.CSVTask_CLASS :
return new CSVTask(m);
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class CSVTask in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.CSVTask_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.CSVTask_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
