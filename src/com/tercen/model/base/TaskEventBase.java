...
public class TaskEventBase extends Event {
	...
	public TaskEventBase(LinkedHashMap m) {
		...
		this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String) m.get(Vocabulary.SUBKIND)
				: (String) (m.get(Vocabulary.KIND) != Vocabulary.TaskEvent_CLASS ? m.get(Vocabulary.KIND) : null);
		this.taskId = (String) m.get(Vocabulary.taskId_DP);
	}

	public static TaskEvent createFromJson(LinkedHashMap m) {
		return TaskEventBase.fromJson(m);
	}

	public static TaskEvent fromJson(LinkedHashMap m) {
		...
		default:
			throw new IllegalArgumentException("bad kind : " + kind + " for class TaskEvent in fromJson constructor");
		}
	}

	public LinkedHashMap toJson() {
		LinkedHashMap m = super.toJson();
		m.put(Vocabulary.KIND, Vocabulary.TaskEvent_CLASS);
		if (this.subKind != null && this.subKind != Vocabulary.TaskEvent_CLASS)
