...
super(m);
this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String)m.get(Vocabulary.SUBKIND)   : (String)(m.get(Vocabulary.KIND) != Vocabulary.Relation_CLASS ? m.get(Vocabulary.KIND) : null);
}

public static Relation createFromJson(LinkedHashMap m) {return RelationBase.fromJson(m);} 
public static Relation fromJson(LinkedHashMap m) {String kind = (String)m.get(Vocabulary.KIND);
switch (kind) {
...
default : throw new IllegalArgumentException(  "bad kind : " + kind +" for class Relation in fromJson constructor");
}
}

public LinkedHashMap toJson() {
LinkedHashMap m = super.toJson();m.put(Vocabulary.KIND, Vocabulary.Relation_CLASS);
if (this.subKind != null && this.subKind != Vocabulary.Relation_CLASS) m.put(Vocabulary.SUBKIND, this.subKind);
