...
public class RunProfileBase extends Profile {
	...
	public RunProfileBase(LinkedHashMap m) {
		...
		this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String) m.get(Vocabulary.SUBKIND)
				: (String) (m.get(Vocabulary.KIND) != Vocabulary.RunProfile_CLASS ? m.get(Vocabulary.KIND) : null);
	...
	public static RunProfile createFromJson(LinkedHashMap m) {
		return RunProfileBase.fromJson(m);
	}

	public static RunProfile fromJson(LinkedHashMap m) {
		String kind = (String) m.get(Vocabulary.KIND);
		switch (kind) {
		case Vocabulary.RunProfile_CLASS:
			return new RunProfile(m);
		default:
			throw new IllegalArgumentException("bad kind : " + kind + " for class RunProfile in fromJson constructor");
		}
	}

	public LinkedHashMap toJson() {
		LinkedHashMap m = super.toJson();
		m.put(Vocabulary.KIND, Vocabulary.RunProfile_CLASS);
		if (this.subKind != null && this.subKind != Vocabulary.RunProfile_CLASS)
