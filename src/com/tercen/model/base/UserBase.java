...
public class UserBase extends Document {
	...
	public UserBase(LinkedHashMap m) {
		...
		this.subKind = m.get(Vocabulary.SUBKIND) != null ? (String) m.get(Vocabulary.SUBKIND)
				: (String) (m.get(Vocabulary.KIND) != Vocabulary.User_CLASS ? m.get(Vocabulary.KIND) : null);
	...
	public static User createFromJson(LinkedHashMap m) {
		return UserBase.fromJson(m);
	}

	public static User fromJson(LinkedHashMap m) {
		...
		case Vocabulary.Team_CLASS:
			return new Team(m);
		default:
			throw new IllegalArgumentException("bad kind : " + kind + " for class User in fromJson constructor");
		}
	}

	public LinkedHashMap toJson() {
		LinkedHashMap m = super.toJson();
		m.put(Vocabulary.KIND, Vocabulary.User_CLASS);
		if (this.subKind != null && this.subKind != Vocabulary.User_CLASS)
